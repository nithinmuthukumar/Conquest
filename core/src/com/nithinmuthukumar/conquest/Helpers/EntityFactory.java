...
public class EntityFactory {
    ...
    public static Entity createBuilding(int x, int y, BuildingData data, GameMap gameMap) {
        ...
        e.add(engine.createComponent(TransformComponent.class).create(x + data.icon.getWidth() / 2, y + data.icon.getHeight() / 2, 0, data.icon.getWidth(), data.icon.getHeight()));
        e.add(engine.createComponent(BuiltComponent.class).create(data));
        gameMap.addLayer(data.tileLayer, x, y, 0);
        BodyComponent body = engine.createComponent(BodyComponent.class).create(x + data.icon.getWidth() / 2, y + data.icon.getHeight() / 2, BodyDef.BodyType.StaticBody);
        for(RectangleMapObject object: data.collisionLayer){
            ...
            createRectFixture(body.body, rect.x - data.icon.getWidth() / 2 + rect.width / 2,
                    rect.y - data.icon.getHeight() / 2 + rect.height / 2, rect.width / 2, rect.height / 2, 0, 0, f, e);
        }
        e.add(body);
